...
%Similar to the approach that we will be adopting later, the sources are assigned a prior distribution to enhance the performance of the separation process.
...
%i.e. the effect of the actions taken at the individual stores and in its local environment could be analyzed. %Furthermore, BSS was used to forecast the U.S. real output and inflation variables \cite{YauR2004}. 
\newpage
...
This has led to some widely used approaches such as Infomax , maximum likelihood estimation , the maximum a posterior (MAP) ,s and FastICA 
...
\end{algorithmic}
\end{algorithm}
\subsubsection{Joint approximation diagonalization of
eigenmatrices (JADE)}
nitialisation. Estimate a whitening matrix V, and set\\
 Form statistics. Est. set of 4th order cumulant matrices:\\
 Optimize an orthogonal contrast. Find the rotation matrix U such that the cumulant matrices are as diagonal as possible (using Jacobi rotations),
that is\\
The separation matrix is therefore obtained by rotation & whitening:
http://personal.ee.surrey.ac.uk/Personal/W.Wang/papers/UDRCSummerSchool2015_Wang_tutorial_SSintroduction.pdf
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{images/dictionary_sparse.png}
...
with complexity $O(n^3)$ (n is the number of channels) but guarantees the optimal assignment to calculate the permutation matrix.
...
\subsubsection{K-SVD}
In Equ.(\ref{equ14}), assuming that both $\mathbf{\Phi}$ and $\mathbf{X}$ is fixed and we only consider one atom $\phi_k$ in dictionary $\mathbf{\Phi}$ and the coefficients $x^k$ ($k^{th}$ row in $\mathbf{X}$) corresponding to it, the penalty term in the objective function can be rewritten as
...
\end{equation}
We define the solution for $\phi_k$ as the first column of $U$ and the coefficient vector $x^k$ as the first column of $V$. The K-SVD algorithm obtains the dictionary update by $K$ separate SVD computations, which explains its name.
...
\subsection{Further research directions in MCA based BSS}
